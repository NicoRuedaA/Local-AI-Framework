...
**Símbolos más frecuentemente olvidados por archivo:**
- `views.py`: `viewsets`, `transaction`, `status`, `permissions`
- `services.py`: `send_mail`, `transaction`, `timezone`, `logging`
- `tasks.py`: `FeatureRequest`, `Vote` (los modelos que usa la task)
- `conftest.py`: `pytest`, `APIClient`, `factory`, modelos con ruta absoluta
...
### 15. Task duplicada en múltiples archivos
**Detectar:** la misma función de task (mismo nombre o misma lógica) definida en más de un archivo del proyecto — por ejemplo en `features/tasks.py` Y en `notifications/tasks.py` Y en `features/services.py`.

**Por qué es crítico:** Celery registra tasks por nombre. Si hay duplicados, solo una versión quedará registrada (según el orden de importación), con comportamiento impredecible. Además, las versiones suelen divergir sutilmente (una verifica `is_public`, otra no), introduciendo bugs difíciles de rastrear.

**Corrección:** la task vive en un único archivo (preferiblemente en la app que "posee" el dominio, ej: `notifications/tasks.py`). Las otras apps la llaman con `.delay()` importándola desde su ubicación canónica:
```python
# features/views.py
from nexo.notifications.tasks import send_notification_on_status_change

# Al cambiar estado:
send_notification_on_status_change.delay(feature_request_id)
```

### 16. App con modelos o vistas no registrada en INSTALLED_APPS
**Detectar:** existe una carpeta de app con `models.py`, `views.py` o `urls.py` que no está en `INSTALLED_APPS` en `settings.py`.

**Consecuencias:** las migrations de esa app no se generan, sus URLs no se incluyen, y sus modelos no son reconocidos por Django.

**Cómo detectarlo:** lista todas las apps del proyecto (subcarpetas con `apps.py` o `models.py`) y verifica que cada una aparece en `INSTALLED_APPS`.

---

## Verificación cruzada obligatoria

Después de revisar cada archivo individualmente, realiza estas comprobaciones que requieren visión del proyecto completo:

**VC-1. Mapa de modelos**
Construye mentalmente la lista: `{nombre_modelo: archivo_donde_está_definido}`.
Si algún nombre aparece más de una vez → error #10 (modelo duplicado).

**VC-2. Origen de User en ForeignKeys**
Para cada ForeignKey a User en cualquier archivo: ¿usa `settings.AUTH_USER_MODEL` o importa `User` directamente?
Si hay un User custom y algún FK usa `auth.User` → error #11.

**VC-3. Mapa de tasks**
Construye la lista: `{nombre_task: [archivos donde está definida]}`.
Si alguna task aparece en más de un archivo → error #15 (task duplicada).

**VC-4. INSTALLED_APPS vs apps existentes**
Lista las apps del proyecto. Compara con `INSTALLED_APPS`.
Si alguna app falta → error #16.
